...
class FeedEntryClass:
    ...
    def __init__(self, name):
        ...
        self.json_path = 'feed/json.txt'
        # set path to the xml file
        self.xml_path = 'feed/xml.txt'
...
class NewsClass(FeedEntryClass):
    ...
    def output(self, message, location):
        ...
        # DB output
        db_obj = DbClass()
        db_obj.db_insert(f"select * from news where text='{self.message}' "
                         f"and location='{self.location}'",
                         f"insert into news (text, location) "
                         f"values ('{self.message}', "
                         f"'{self.location}');")
...
class AdvClass(FeedEntryClass):
    ...
    def output(self, message, expiration_date):
        ...
        # DB output
        db_obj = DbClass()
        db_obj.db_insert(f"select * from ads where text='{self.message}' "
                         f"and date='{self.expiration_date}'",
                         f"insert into ads (text, date) "
                         f"values ('{self.message}', "
...
class WeatherForecastClass(FeedEntryClass):
    ...
    def output(self, location, summary, temperature):
        ...
        # DB output
        db_obj = DbClass()
        db_obj.db_insert(f"select * from forecasts where location='{self.location}' "
                         f"and summary='{self.message}'"
                         f"and temperature='{self.temperature}{self.degree_sign}C'",
                         f"insert into forecasts (location, summary, temperature) "
                         f"values ('{self.location}', "
                         f"'{self.message}', '{self.temperature}{self.degree_sign}C');")
        # print status if manual load
        if user_interface.top_menu_input == '1':
            # write to console status message
            self.status(self.name)
            # clear screen
...
# Class for json batch load
class JsonClass:
    def __init__(self, path):
        self.path = path
        self.json_batch_file = ''
        self.file_text = ''
        self.json_dict = []
        self.records_number = 0
        self.invalid_record = 0
    ...
    # set default path to a batch json file
    def get_path(self):
        if self.path == '':
            self.path = 'feed/json.txt'
    ...
    def open_dir(self):
        ...
        try:
            ...
            if os.path.getsize(self.path) == 0:
                ...
                # open file for write
                self.json_batch_file = open(f'{self.path}', 'r')
                self.file_text = self.json_batch_file.read()
                # close file to see results immediately
                self.json_batch_file.close()
        # process FileNotFoundError exception
        except FileNotFoundError:
            # print error message
            print(f"Error: Can't open target file {self.path}")
    ...
    def json_batch_parser(self, json_dict):
        # define directory
        self.file_dir = os.path.dirname(f'{self.path}')
        self.json_dict = json_dict
        # get multiple record dictionaries
        for sub_dict in self.json_dict['head']:
            # define post type
            if sub_dict['type'] == 'Weather forecast':
                # create WeatherForecastClass class object
                wfc = WeatherForecastClass('Weather forecast')
                try:
                    # call wfc.output method
                    wfc.output(sub_dict['location'], sub_dict['summary'], sub_dict['temperature'])
                    self.records_number += 1
                except IndexError:
                    print('Error: Incorrect record format in a batch file')
                    self.invalid_record += 1
                    # write to invalid file
                    self.invalid_file = open(f'{self.file_dir}/invalid_records.txt', 'a')
                    self.invalid_file.write(f'{feed.datetime_string}\t{sub_dict}\n')
                    self.invalid_file.close()
            # define post type
            elif sub_dict['type'] == 'Private Ad':
                # create AdvClass class object
                adv = AdvClass('Private Ad')
                try:
                    # capitalize function
                    self.par2 = module4.capitalize_sentences(sub_dict['text'])
                    # verify date
                    vrf = VerificationClass()
                    if not vrf.verify_date(sub_dict['date']):
                        # write to invalid file
                        self.invalid_file = open(f'{self.file_dir}/invalid_records.txt', 'a')
                        self.invalid_file.write(f'{feed.datetime_string}\t{sub_dict}\n')
                        self.invalid_file.close()
                    ...
                    self.invalid_record += 1
            # define post type
            elif sub_dict['type'] == 'News':
                # create NewsClass class object
                nws = NewsClass('News')
                try:
                    # capitalize function
                    self.par2 = module4.capitalize_sentences(sub_dict['text'])
                    self.par3 = sub_dict['location']
                    # call news.output method
                    nws.output(self.par2, self.par3)
                    self.records_number += 1
                except IndexError:
...
class XmlClass:
    ...
    # set default path to a batch xml file
    def get_path(self):
        if self.path == '':
            self.path = 'feed/xml.txt'
    ...
    def open_dir(self):
        ...
        try:
            ...
            if os.path.getsize(self.path) == 0:
                ...
                # open file for write
                self.xml_file = Et.parse('feed/xml.txt')
        # process FileNotFoundError exception
        except FileNotFoundError:
            # print error message
            print(f"Error: Can't open target file {self.path}")
    ...
    def xml_batch_parser(self, xml_file):
        # define directory
        self.file_dir = os.path.dirname(f'{self.path}')
        ...
        for xml_record in root.findall('record'):
            # define post type
            if xml_record.find('type').text == 'Weather forecast':
                self.par2 = xml_record.find('location').text
                self.par3 = xml_record.find('summary').text
                self.par4 = xml_record.find('temperature').text
                # create WeatherForecastClass class object
                wfc = WeatherForecastClass('Weather forecast')
                try:
                    # call wfc.output method
                    wfc.output(self.par2, self.par3, self.par4)
                    self.records_number += 1
                    ...
                    # write to invalid file
                    self.invalid_file = open(f'{self.file_dir}/invalid_records.txt', 'a')
                    self.invalid_file.write(f'{feed.datetime_string}\t{xml_record}\n')
                    self.invalid_file.close()
            # define post type
            elif xml_record.find('type').text == 'Private Ad':
                # create AdvClass class object
                adv = AdvClass('Private Ad')
                try:
                    # capitalize function
                    self.par2 = xml_record.find('text').text
                    # verify date
                    vrf = VerificationClass()
                    if not vrf.verify_date(xml_record.find('date').text):
                        # write to invalid file
                        self.invalid_file = open(f'{self.file_dir}/invalid_records.txt', 'a')
                        self.invalid_file.write(f'{feed.datetime_string}\t{xml_record}\n')
                        self.invalid_file.close()
                        ...
                        # set parameter values
                        self.par3 = xml_record.find('date').text
                        # call adv.output method
                        adv.output(self.par2, self.par3)
                        self.records_number += 1
                except AttributeError:
                    ...
                    self.invalid_record += 1
            # define post type
            elif xml_record.find('type').text == 'News':
                # create NewsClass class object
                nws = NewsClass('News')
                try:
                    # capitalize function
                    self.par2 = module4.capitalize_sentences(xml_record.find('text').text)
                    self.par3 = xml_record.find('location').text
                    # call news.output method
                    nws.output(self.par2, self.par3)
                    self.records_number += 1
                except AttributeError:
...
class DbClass:
    def __init__(self):
        self.sql_select_string = ''
        self.sql_insert_string = ''
        ...
        with pyodbc.connect('DRIVER={SQLite3 ODBC Driver};Direct=True;'
            ...
            self.cursor.execute('CREATE TABLE IF NOT EXISTS news (text CHAR, location CHAR);')
            self.cursor.execute('CREATE TABLE IF NOT EXISTS ads (text CHAR, date DATE);')
            self.cursor.execute('CREATE TABLE IF NOT EXISTS forecasts (location CHAR, summary CHAR, temperature CHAR);')

    def db_select(self, sql_select_string):
        self.sql_select_string = sql_select_string
        self.cursor.execute(self.sql_select_string)
        return self.cursor.fetchall()

    def db_insert(self, sql_select_string, sql_insert_string):
        self.sql_select_string = sql_select_string
        self.sql_insert_string = sql_insert_string
        self.cursor.execute(self.sql_select_string)
        self.duplicate_result = self.cursor.fetchall()
        if not self.duplicate_result:
            self.cursor.execute(self.sql_insert_string)
        self.cursor.commit()
        self.cursor.close()
...
class UIClass:
    ...
    def __init__(self):
        ...
        self.json_batch_file_content = {}
        self.xml_file = ''

    # select menu option method
    def select_option(self):
        while self.menu_input not in ('0', '1', '2', '3', '4'):
            # print menu
            print("-" * 20)
            print("Select input type:")
            print("-" * 20, "\n")
            print("Press '1' to input feed entry manually")
            print("Press '2' for batch load from txt file")
            print("Press '3' for batch load from json file")
            print("Press '4' for batch load from xml file")
            ...
            if self.top_menu_input == '1':
                ...
                self.menu_input = '-1'
            # batch load from txt file
            elif self.top_menu_input == '2':
                self.path_input = input(f"\nInput path to a file or press enter for "
                                        f"default path value {feed.path}\n> ")
                # create object of BatchLoadClass
                bth = BatchLoadClass(self.path_input)
                # get default path for empty input
                bth.get_path()
                # check directory
                self.batch_file_content = bth.open_dir()
                # parse batch file
                bth.batch_parser(self.batch_file_content)
            # batch load from json file
            elif self.top_menu_input == '3':
                self.json_path_input = input(f"\nInput path to a file or press enter for "
                                             f"default path value {feed.json_path}\n> ")
                # create object of JsonClass
                jsn = JsonClass(self.json_path_input)
                # get default path for empty input
                jsn.get_path()
                # check directory
                jsn.open_dir()
                # get json context
                self.json_batch_file_content = json.load(open(f'{jsn.path}', 'r'))
                # parse json context
                jsn.json_batch_parser(self.json_batch_file_content)
            elif self.top_menu_input == '4':
                self.xml_path_input = input(f"\nInput path to a file or press enter for "
                                            f"default path value {feed.xml_path}\n> ")
                # create object of JsonClass
                xml_obj = XmlClass(self.xml_path_input)
                # get default path for empty input
                xml_obj.get_path()
                # check directory
                xml_obj.open_dir()
                try:
                    # get xml context
                    self.xml_file = Et.parse(f'{xml_obj.path}')
                except Et.ParseError:
                    # print error message
                    print(f"Error: Can't parse target file {xml_obj.path}")
                    input("Press ENTER to quit")
                    # stop the script
                    quit()
                # parse json context
                xml_obj.xml_batch_parser(self.xml_file)
            # quit
